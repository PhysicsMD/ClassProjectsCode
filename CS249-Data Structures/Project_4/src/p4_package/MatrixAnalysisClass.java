...
public class MatrixAnalysisClass
   ...
   /**
    ...
    {
       CellDataClass currentNumber = new CellDataClass(0, xIndex, yIndex);
       displayStatus(recLevel, "Trying Location: ", currentNumber, TEST_LOCATION);
       iterationCount += 1;
       if(!isInBounds(xIndex, yIndex)) 
       {
          displayStatus(recLevel, "Location failed: ", null, OUT_OF_BOUNDS);
          return false;
       }
       int currentVal = dataArray[yIndex][xIndex];
       currentNumber.setCellData(currentVal,xIndex, yIndex);
       if(locationSet.hasItem(currentNumber)) 
       {
          displayStatus(recLevel, "Location failed: ", currentNumber, INVALID_ITEM);
          return false;
       }
       if(runningTotal+currentVal > sumRequest) 
       {
          displayStatus(recLevel, "Location failed: ", currentNumber, OVER_SUM);
          return false;
       }
       else
       {
          runningTotal += currentVal;
          displayStatus(recLevel, "Valid Location found: ", currentNumber, VALID_ITEM);
       ...
       if(findSumHelper(sumRequest, runningTotal, xIndex+1, yIndex, recLevel)) 
       {
          displayStatus(recLevel, "Item found: ", currentNumber, VALID_ITEM);
          return true;
       }
       else if(findSumHelper(sumRequest, runningTotal, xIndex, yIndex+1, recLevel)) 
       {
          displayStatus(recLevel, "Item found: ", currentNumber, VALID_ITEM);
          return true;
       }
       else if(findSumHelper(sumRequest, runningTotal, xIndex-1, yIndex, recLevel)) 
       {
          displayStatus(recLevel, "Item found: ", currentNumber, VALID_ITEM);
          return true;
       }
       ...
       {
          displayStatus(recLevel, "Location failed: ", currentNumber, NO_SOLUTION);
